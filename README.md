...
Для того, чтобы поднять кластер, включающий в себя master и replica node, нужно запустить ansible playbook:

![image](https://github.com/user-attachments/assets/a0b77b4d-cd7d-4f03-844b-1045d041c178)

Все будет корректно настроено, благодаря данным условиям в конце блоков с задачами:
...
Это позволит выполнять задачи только для определенных хостов, так как настройка каждого из них отличается. После выполнения playbook, получим правильно настроенный кластер, работоспособность которого можно увидеть ближе к концу отчета
